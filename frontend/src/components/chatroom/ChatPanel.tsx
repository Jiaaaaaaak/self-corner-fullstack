            ...
            >
              {msg.role === "student" && (
                <div className="w-8 h-8 rounded-full bg-white/80 backdrop-blur-sm border border-[#E5E2D9]/50 overflow-hidden shrink-0 mr-2.5 self-end shadow-sm">
                  <img src={`/avatars/${studentName}.png`} alt={studentName} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; e.currentTarget.parentElement!.innerHTML = `<span class="text-[10px] font-bold text-[#706C61] flex items-center justify-center w-full h-full">${studentName.charAt(0)}</span>`; }} />
                </div>
              )}
              <div
          ...
          {isThinking && (
            <div className="flex justify-start animate-in fade-in duration-300">
              <div className="w-8 h-8 rounded-full bg-white/80 backdrop-blur-sm border border-[#E5E2D9]/50 overflow-hidden shrink-0 mr-2.5 self-end shadow-sm flex items-center justify-center">
                <img src={`/avatars/${studentName}.png`} alt={studentName} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.replaceWith(Object.assign(document.createElement('div'), { className: 'w-3.5 h-3.5', innerHTML: '' })); }} />
              </div>
