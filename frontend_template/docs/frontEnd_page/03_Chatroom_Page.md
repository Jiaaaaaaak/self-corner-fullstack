...
### 1.3 學生特質選擇 (Personality Selection)
*   **功能**: 選擇情境後，提供 10 種特定的性格類型（如：防衛刺蝟型、衝動干擾型等）供使用者挑選。
*   **UI 呈現**: 彈出對話框或遮罩畫面，上方包含「上一步」與「關閉」按鈕。選項以 Grid 按鈕排列。

### 1.4 學齡選擇 (Age Group Selection)
*   **功能**: 選擇學生特質後，提供 4 個學齡階段（國小低年級、中年級、高年級、國中）。
*   **UI 呈現**: 延續選擇視圖風格，提供大面積按鈕供點擊，點擊後進入對話（或語音開啟確認提示）。
