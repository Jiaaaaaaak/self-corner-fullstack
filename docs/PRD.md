...
- **情境選擇**：預設從 10 個 SEL 情境中隨機顯示 6 張情境卡（`ScenarioCard`），可點擊「換一批」重新抽取。點擊情境卡後展開詳情（`ScenarioDetail`）。
- **學生特質選擇**：確認情境後，進入「學生特質」選擇畫面，提供 10 種特定的性格類型（如：防衛刺蝟型、衝動干擾型、焦慮退縮型等），這將決定 AI 學生的原始性格與溝通風格。
- **學齡選擇**：最後選擇學生的「學齡」（國小低年級、中年級、高年級、國中），這將影響 AI 學生的用語習慣、理解能力與表達深度。
- 確認所有設定後點擊「開始」，系統將根據上述選擇動態組裝 AI 學生的 system prompt 並建立 Session。
- 亦可透過「隨機挑戰」快速進入對話。
