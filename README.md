...
>
> 涵蓋：環境需求、venv 建置位置、資料庫初始化、三個終端機的啟動順序、常見問題排解。
...
- 後端版本：1.3.0（新增 `GET /session/count`；`SessionResponse` 新增 `student_name`；全端點 UTC 時區修正附加 `+00:00`）
- 前端版本：2.1（FeedbackTabs 元件；Sidebar/ChatPanel/Chatroom/Info 介面更新）
