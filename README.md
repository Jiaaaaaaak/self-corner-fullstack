...
│   │
│   ├── api/                       # REST API 路由層
│   │   ├── auth.py                # 認證（登入 / 註冊 / 刷新 / 登出 / Google OAuth）
...
- 後端版本：1.4.0（新增 Google OAuth 登入；新增 `FRONTEND_URL` 環境變數；新增 `services/oauth.py`）
- 前端版本：2.2（新增 `AuthInitializer` 頁面載入自動恢復登入狀態；Google OAuth 登入整合）
